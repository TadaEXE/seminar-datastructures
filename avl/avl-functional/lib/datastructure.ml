...
let rec split_max tree = match tree with
  Leaf -> (Leaf, 0) |
  Node (h, a, left, Leaf) -> (left, a) |
  Node (h, a, left, right) -> let (r1, a1) = split_max right in (balL (Node (max (ht left) (ht r1) + 1, a, left, r1)), a1)

let rec delete x tree = match tree with
  Leaf -> Leaf |
  Node (h, a, left, right) -> 
    if x < a then let new_left = delete x left in 
      balR (Node (max (ht new_left) (ht right) + 1, a, new_left, right))
  else if x > a then let new_right = delete x right in 
      balL (Node (max (ht left) (ht new_right) + 1, a, left, new_right))
  else if left == Leaf then right
  else let (l1, a1) = split_max left in balR (Node (max (ht l1) (ht right) + 1, a1, l1, right))
