...
void benchmarkVec(char* path, AvlVec* avl) {
    bool isInsertion = true;
    std::ifstream file(path);
    std::vector<int> sequence = nextSequence(&file);
    int count = 0;
...
void clearBenchmark(int n) {
    ...
    for (int i = 0; i < inserts.size(); i++) {
        avlRec = insert(avlRec, inserts[i]);
    }
    auto t1 = clock();
    std::cout << "Insert: " << t1 - t0 << "\n";
    for (int i = 0; i < deletes.size(); i++) {
        avlRec = deleteNode(avlRec, deletes[i]);
    }
    auto t2 = clock();
...
"../avl-functional/data/range_33554431.txt",
    ...
    for (int n = 0; n < 10; n++) {
        ...
        benchmark(paths[n], new AvlRec(root1));
        std::cout << "Recursive: \n";
